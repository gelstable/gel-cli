...
from . import assets, registry_manifest
...
def package_manager_version(version: str) -> str:
    """Use distro ordering: a prerelease sorts below its plain version."""
    registry_manifest.release_channel(version)
    return version.split("+", 1)[0].replace("-", "~", 1)
...
def deb_command(target: assets.Target, version: str, completions_dir: Path) -> list[str]:
    ...
    return [
        ...
        "--no-strip",
        "--deb-version",
        f"{package_manager_version(version)}-1",
        "--target",
        target.triple,
        "--output",
...
def rpm_command(target: assets.Target, version: str) -> list[str]:
    ...
    return [
        ...
        "generate-rpm",
        "--set-metadata",
        f'version = "{package_manager_version(version)}"',
        "--target",
        target.triple,
        "--arch",
        target.rpm_arch,
