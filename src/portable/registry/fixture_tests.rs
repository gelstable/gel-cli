...
/// The release manifest generated by `scripts/release/registry_manifest.py` is
/// consumed by gel-registry, which renders its index fragments into the index
/// documents this client parses. Parse the committed generator output with the
/// client's own index parser so a generator change that the client cannot read
/// fails here rather than in production.
#[test]
fn generated_release_manifest_fragments_parse_as_index_documents() {
    use super::index::IndexDocument;

    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/registry/release-manifest/gel-registry.json");
    let manifest: serde_json::Value =
        serde_json::from_slice(&fs::read(&path).expect("read release manifest"))
            .expect("release manifest is valid JSON");

    let indexes = manifest["indexes"].as_array().expect("indexes array");
    assert_eq!(indexes.len(), 5, "one fragment per registry target");

    let source = Source::File(path.clone());
    for fragment in indexes {
        let platform = fragment["platform"].as_str().expect("platform");
        assert_eq!(fragment["channel"].as_str(), Some("stable"));

        let document_bytes =
            serde_json::to_vec(&serde_json::json!({ "packages": fragment["packages"] }))
                .expect("serialize index document");
        let document = IndexDocument::from_slice(&document_bytes)
            .unwrap_or_else(|error| panic!("{platform}: {error}"));

        let artifacts = document
            .validate(&source, &source, platform)
            .unwrap_or_else(|error| panic!("{platform}: {error}"))
            .into_artifacts();
        ...
        assert_eq!(
            artifacts.len(),
            1,
            "{platform}: one CLI artifact selected from the two installrefs"
        );
        assert!(
            artifacts[0].url().as_str().ends_with(".zst"),
            "{platform}: the zstd installref must win selection, got {}",
            artifacts[0].url()
        );
    }
}
